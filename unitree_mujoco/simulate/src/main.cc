...
namespace
{
  ...
  void PhysicsLoop(mj::Simulate &sim)
  {
    ...
    while (!sim.exitrequest.load())
    {
      ...
      m->opt.timestep = 0.001; // 0.002; // sim step time, dt
...
{
  // ros::init(argc, argv, "unitree_mujoco_node");
  // ros::NodeHandle nh;
  ros::NodeHandle& nh = *static_cast<ros::NodeHandle*>(arg);
  ...
  UnitreeSdk2Bridge unitree_interface(m, d, nh);
...
int main(int argc, char **argv)
{
  ...
  // YAML::Node yaml_node = YAML::LoadFile("src/unitree_mujoco/simulate/config.yaml");
  YAML::Node yaml_node = YAML::LoadFile("/home/docker_dev/isaccsim_mujoco/src/unitree_mujoco/simulate/config.yaml");
  ...
  string scene_path = "/home/docker_dev/isaccsim_mujoco/src/unitree_mujoco/unitree_robots/" + config.robot + "/" + config.robot_scene;
  ...
  int rc = pthread_create(&unitree_thread, NULL, UnitreeSdk2BridgeThread, &nh);
  if (rc != 0)
  {
