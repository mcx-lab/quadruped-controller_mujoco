...
void ConvexMPCLocomotion::_SetupCommand(ControlFSMData<float> & data){
  if(data._quadruped->_robotType == RobotType::UNITREE_A1){
    // _body_height = 0.29;
    _body_height = 0.31; //unitree_go1
    // _body_height = 0.25; 
  }else if(data._quadruped->_robotType == RobotType::UNITREE_ALIENGO){
    _body_height = 0.40;
  }else{
    assert(false);
  }
...
void ConvexMPCLocomotion::solveDenseMPC(int *mpcTable, ControlFSMData<float> &data) {
  ...
  float Q[12] = {0.25, 0.25, 10, 5, 5, 20, 0, 0, 0.3, 0.2, 0.2, 0.1}; //unitree_go1
  // float Q[12] = {0.25, 0.25, 20, 2, 2, 50, 0, 0, 0.3, 0.2, 0.2, 0.1}; //origin
