...
void WBC_Ctrl<T>::_UpdateLegCMD(ControlFSMData<T> & data){
  ...
  //   {
  //     pretty_print(data._legController->datas[leg].q, std::cout, "leg " + std::to_string(leg));
  //   }

  //   printf("\nJoint velocity data\n");
  //   for (int leg=0; leg<4; ++leg)
  //   {
  //     pretty_print(data._legController->datas[leg].qd, std::cout, "leg " + std::to_string(leg));
  //   }
  ...
  //   for (int leg=0; leg<4; ++leg)
  //   {
  //     pretty_print(cmd[leg].tauFeedForward, std::cout, "leg " + std::to_string(leg));
  //   }

  //  printf("\nJoint position command\n");
  //   for (int leg=0; leg<4; ++leg)
  //   {
  //     pretty_print(cmd[leg].qDes, std::cout, "leg " + std::to_string(leg));
  //   }
  ...
  //   for (int leg=0; leg<4; ++leg)
  //   {
  //     pretty_print(cmd[leg].qdDes, std::cout, "leg " + std::to_string(leg));
  //   }
}
...
void WBC_Ctrl<T>::_UpdateModel(const StateEstimate<T> & state_est, 
  ...
  // pretty_print(_state.bodyOrientation, std::cout, "bodyOrientation");
  // pretty_print(_state.bodyPosition, std::cout, "bodyPosition");
  // pretty_print(_state.bodyVelocity, std::cout, "bodyVelocity");
  // pretty_print(_state.q, std::cout, "q");
  // pretty_print(_state.qd, std::cout, "qd");

  // _model.printState();

  // printf("\n--------------INPUT FOR OPTIMIZER--------------\n");
  // pretty_print(_A, std::cout, "_A");
  // pretty_print(_Ainv, std::cout, "_Ainv");
  // pretty_print(_coriolis, std::cout, "_coriolis");
