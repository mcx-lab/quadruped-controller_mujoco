...
void FSM_State_StandUp<T>::run() 
{
  ...
  if(this->_data->_quadruped->_robotType == RobotType::MINI_CHEETAH) 
  {
    ...
    for(int i = 0; i < 4; i++) {
      this->_data->_legController->commands[i].kpCartesian = Vec3<T>(750, 750, 750).asDiagonal(); //700
      this->_data->_legController->commands[i].kdCartesian = Vec3<T>(15, 15, 15).asDiagonal();   //15
