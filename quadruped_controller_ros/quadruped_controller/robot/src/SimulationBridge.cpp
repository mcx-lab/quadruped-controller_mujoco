...
void SimulationBridge::init()
{
    ...
    _gazeboCommunicationChannel = new GazeboCommunicationChannel(robotType, &nh, &_lowCmd, &_lowState, &twist);
    ROS_INFO("Gazebo Communication Channel Created");
...
void SimulationBridge::run()
{
    ...
    std::cout << "Terminating gazebo thread\n";
    gazeboThread.join();
...
void SimulationBridge::pubGazebo()
{
    while(ros::ok())
    {
        _gazeboCommunicationChannel->sendServoCmd();
...
{
    delete _gazeboCommunicationChannel;
    delete _robotRunner;
}
